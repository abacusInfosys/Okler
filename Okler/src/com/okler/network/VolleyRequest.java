...
public class VolleyRequest {
	...
	public static boolean addJsonArrayRequest(Context paramContext,
			WebJSonArrayReqest jsonArrayRequest) {
		try {
			if (NetworkUtils.checkNetworkAvailable(paramContext)) {
				getRequestQueue(paramContext).add(jsonArrayRequest);
				return true;
			} else
				Toast.makeText(paramContext, "Check network connection",
	...
	public static boolean addJsonObjectRequest(Context paramContext,
	...
	public static boolean addBooleanRequest(Context paramContext,
			WebBooleanRequest boolRequest) {
		try {
			if (NetworkUtils.checkNetworkAvailable(paramContext)) {
	...
	public static Boolean addStringRequest(Context paramContext,
			WebStringRequest boolRequest) {
		try {
			if (NetworkUtils.checkNetworkAvailable(paramContext)) {
				getRequestQueue(paramContext).add(boolRequest);
				return true;
