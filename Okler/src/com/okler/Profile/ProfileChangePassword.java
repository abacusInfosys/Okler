			...
			public void onClick(View v) {
				...
				if (tval.validateText("Please Enter old Password")) {
					...
					if (tval.ValidateMinimumChars(
							"Please Enter atleast 6 characters", 6)) {
						flag = false;
						tval = new TextValidations(edtTxt_retype_password);
						if (tval.validateText("Please retype new password")) {
							flag = false;
							...
							if (!new_pass.equals(retype_pass)) {
								...
								edtTxt_retype_password
										.setError("Retyped Password should be equal new Password");
								flag = false;
							} else {
								if (old_pass.equals(new_pass)
									...
									edtTxt_retype_password.setText("");
									edtTxt_new_password
											.setError("Old and New Password should not be same");
				...
				if (flag) {
					...
					if (flag) {
						String change_pass = getString(R.string.change_password_url)
								+ userid
								+ "&newpwd="
								+ newPwd1
								+ "&oldpwd="
