...
public class ProfileNewAddress extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (flag == true) {
			TextView text_title = (TextView) toolBar
		...
		dobTV.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				dobTV.setText("");
				fromDatePickerDialog = new DatePickerDialog(act,
						new OnDateSetListener() {
							...
							public void onDateSet(DatePicker view, int year,
								...
								if ((newDate.compareTo(newCalendar) == -1)
									...
									String ddd = dateFormatter.format(newDate
											.getTime());
		...
		button_saveinfo.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case 1:
					...
					// Toast.makeText(getApplicationContext(), "SAVGE CLICKED",
		...
		if (Okler.getInstance().getBookingType() == 9
				|| Okler.getInstance().getBookingType() == 20) {
		...
		if (content == 2) {
			...
			try {
				...
				String r_id = addbean.optString("relationId");
				if (r_id == null || r_id.equals("null") || r_id.equals("")
						|| r_id == "0" || r_id.equals("0")) {
					r_id = "1";
				}
				rel_id = Integer.parseInt(r_id);
				String g_id = "";

				g_id = addbean.optString("genderId");
				if (g_id == null || g_id.equals("null") || g_id.equals("")
						|| g_id.equals("m")) {
					g_id = "0";
				}
			...
			Utilities.setTitleText(toolBar, "Edit Address");
		}
		getStates();
		states = Okler.getInstance().getStates();
		adapterSt = new ArrayAdapter<String>(act,
		...
		spinnerGender.setSelection(gid);
		/*
		 * stateTv.setOnFocusChangeListener(new OnFocusChangeListener() {
		 * 
		 * @Override public void onFocusChange(View v, boolean hasFocus) { //
		 * TODO Auto-generated method stub String city =
		 * stateTv.getText().toString(); if(city.equals(" ") != false) {
		 * Toast.makeText(getApplicationContext(), "city is:"+city,
		 * Toast.LENGTH_LONG).show(); int id1 = states.indexOf(city);
		 * Toast.makeText(getApplicationContext(), "id1:"+id1,
		 * Toast.LENGTH_LONG).show(); state_id = state_id_list.get(id1);
		 * Toast.makeText(getApplicationContext(), "city id:"+state_id,
		 * Toast.LENGTH_LONG).show();
		 * 
		 * } } });
		 */
	...
	public void addNewAddress() {
		 ...
		 * city = cityTV.getText().toString();
		 * 
		 * if(!(cities.size()<=0)){ cid = cities.indexOf(city);
		 * if(!(city_id_list.size()<=0)) cityId = city_id_list.get(cid); }
		 */
		...
		if (tval.ValidateMinimumChars("Enter a valid name", 3)) {
			...
			if (tval.ValidateMinimumChars("Enter a Valid Surname", 3)) {
				...
				if ((dob2.equals(""))) {
					Toast.makeText(act, "Please Enter Your Birthdate",
							Toast.LENGTH_LONG).show();
				} else {
					...
					if (tval.validateText("Please Enter a valid Address")) {
						...
						if (tval.validateText("Please Enter a valid Address")) {
							...
							if (uiUtils.validateSpinner(
									"Please Select Relation", "Relation",
									spinnerRelation.getSelectedItem()
											.toString(), act)) {
								relation = (String) spinnerRelation
										.getSelectedItem();
								this.relation = Okler.getInstance()
										.getRelationList();
								if (!(this.relation.size() <= 0)) {
									rid = this.relation.indexOf(relation);
								}
								relation_id = Okler.getInstance()
										.getRelationIdList();
								if (!(relation_id.size() <= 0))
									rel_id = Integer.parseInt(relation_id
											.get(rid - 1));
								int r = rel_id;
								uiUtils = new UIUtils(spinnerGender);
								if (uiUtils.validateSpinner(
										"Please Select Gender", "Gender",
										spinnerGender.getSelectedItem()
												.toString(), act)) {
									gender = (String) spinnerGender
											.getSelectedItem();
									...
									if (tval.validateText("Please Enter a Landmark")) {
										...
										if (tval.validatePinCode("Please Enter a Valid Pincode")) {
											...
											if (tval.validateMobile("Please Enter a Valid Mobile number")) {
												mobile = mobileTV.getText()
														.toString();
												uiUtils = new UIUtils(stateTv);
												String st = stateTv.getText()
														.toString();
												if (uiUtils
														.validateState(
		...
		if (Okler.getInstance().getBookingType() == 9
				|| Okler.getInstance().getBookingType() == 20) {
			add_address = getString(R.string.AddNewPatientUrl)
					+ user_id
					...
					+ dob
					+ getString(R.string.relation_id)
					+ rel_id
					+ getString(R.string.gender)
					+ gid
					+ ""
					+ getString(R.string.mobileno)
					+ mobile
					+ getString(R.string.addr1)
					+ address1
					+ getString(R.string.addr2)
					+ address2
					+ getString(R.string.land_mark)
					+ landmark1
					+ getString(R.string.pincode) 
					+ pincode 
					+ getString(R.string.state_id)
					+ state_id 
					+ getString(R.string.getMedsUrlCityId) 
					+ city_id;
		} else {
			add_address = getResources()
					.getString(R.string.add_patient_address)
					...
					+ pincode
					+ getString(R.string.country_id)
					+ ""
					+ getString(R.string.zone_id)
					+ ""
					+ getString(R.string.state_id)
					+ state_id 
					+ getString(R.string.city_id) + city_id;
		}
		if (flag) {
			flag = false;
			Log.e("URL", add_address);
						...
						public void onResponse(JSONObject response) {
							...
							try {
								...
								if (messaqge.equals("inserted successfully")) {
									...
									String default_bill_ship_url = getString(R.string.DefaultBillShipPart1)
											+ user_id
											+ "&ship_id="
											+ ship
											+ "&bill_id=" + bill;
									...
									WebJsonObjectRequest webjson = new WebJsonObjectRequest(
											Method.GET, default_bill_ship_url,
											new JSONObject(),
									...
									VolleyRequest.addJsonObjectRequest(
											getApplicationContext(), webjson);
								}

								// Toast.makeText(getApplicationContext(),
								// messaqge, Toast.LENGTH_LONG).show();
							...
							if (Okler.getInstance().getBookingType() == 9
									|| Okler.getInstance().getBookingType() == 20) {
								intent = new Intent(ProfileNewAddress.this,
										SelectPatientInfoActivity.class);
								startActivity(intent);
							} else {
								intent = new Intent(ProfileNewAddress.this,
			...
			Toast.makeText(act, "Please Enter Valid Address", Toast.LENGTH_LONG)
					.show();
		}
	}
	...
	public void getRelation() {
		...
		WebJsonObjectRequest webjson = new WebJsonObjectRequest(Method.GET,
				getResources().getString(R.string.relationUrl),
				new JSONObject(), new Listener<JSONObject>() {
					...
					public void onResponse(JSONObject response) {
						...
						try {
							...
							spinnerRelation.setSelection(rel_id);
