...
public class ProductCheckoutNewAddress extends BaseActivity {
	...
	private Button notifCount,save;
	 ImageView imgBack;
	 EditText nameTV,surnameTV,addr1TV,addr2TV,pincodeTV,et_prefferedDelTime;
	 ...
	 if(content == 2)
		{
			...
			try {
				...
				et_prefferedDelTime.setText(addbean.optString("preferred_del_time"));
	...
	/*	}
	 ...
	 edtTxt_state.setOnFocusChangeListener(new OnFocusChangeListener() {
	 ...
	 cityTV.setOnFocusChangeListener(new OnFocusChangeListener() 
		...
		public void addNewAddress()
		{
			...
			int user_id = ubean.getId();
			String state_name, city_name, name, surname, dob, add1, add2, relation, gender, landmark, pincode, mobile, state;
			...
			int id = 0;
			if(!(cities.size()<=0)){
			id = cities.indexOf(city);
			if(!(city_ids.size()<=0))
			city_id = city_ids.get(id);
			city = city_id;
			}
			
			mobile = "9876543210";
			state = ""+state_id; 
			city_name = cityTV.getText().toString();
			state_name = edtTxt_state.getText().toString();
			
			String add_address = getResources().getString(R.string.add_patient_address) + "customer_id="+ user_id + "&company="+ "" 
					+"&customer_name=" + name +"&surname=" + surname + "" + "&dob=" + "" + "&gender=" + ""
					+"&email=" + ubean.getEmail() + "&mobileNo=" + ubean.getPhone() + "&addr1=" + add1 + "&addr2=" + add2 + "&landmark=" + "" + "&zip="+pincode
					+ "&country_id=" + "99" + "&zone_id=" + "" + "&state_id=" + state + "&city_id=" + city_id; 
			
			UsersDataBean curbean = cdbean.getCurUBean();
			ArrayList<AddressDataBean> adList = curbean.getAddDatabean();
			AddressDataBean abean = new AddressDataBean();
			abean.setFirstname(name);
			abean.setLastname(surname);
			abean.setAddr_id(add1);
			abean.setAddress2(add2);
			abean.setZip(pincode);
			abean.setState(state_name);
			abean.setCity(city_name);
			abean.setDefault_shipping(check++);
			adList.add(abean);
			ubean.setAddDatabean(adList);
			cdbean.setUbean(ubean);
		...
		public void getCity2()
		{
			...
			int state_id = Integer.parseInt(add.getState_id());
						...
						public void onResponse(JSONObject response) 
						{
							...
							for(int i = 0 ; i < doctorsArr.length();i++)
							{
								try
								{
									...
									city_ids.add(docObj.getString("id"));
		...
		public void editAddress()
		{
			...
			//int user_id = ubean.getId();
			user_id = ubean.getId();
			String name="",name1="",pref_del_time="",pref_del_time1="", surname="",surname1="", dob="", add1="", add2="",email="",address1="",address2="",landmark1="", country_id="",relation="", zone_id="",gender="", landmark="", pincode="", mobile="", city="", state="";
			flag = false;
			...
			if(uiUtils.validateState("Please Enter a Valid State", state1, ack)){
				state = ""+state_id;
			...
			if(uiUtils.validateCity("Enter a Valid City", city1, ack)){
				if(!(cities.size()<=0)){
			...
			if(tval.validateText("Please Enter Preferred Time")){
				adbean.setPreferred_del_time(et_prefferedDelTime.getText().toString());
				pref_del_time=et_prefferedDelTime.getText().toString();
				flag=true;
			}
			}
			}
			...
			if(content==2){
			add_address = getResources().getString(R.string.add_patient_address)+ "addr_id=" 
			+ adId + "&customer_id="+ user_id + "&company="+ ""+"&customer_name=" + name1 +"&surname=" 
			+ surname1  +"&email=" + email + "&mobileNo=" + mobile + "&addr1=" + address1 + "&addr2=" 
			+ address2 + "&landmark=" + landmark1 + "&zip="+pincode+ "&country_id=" + country_id 
			+ "&zone_id=" + zone_id+ "&state_id=" + state + "&city_id=" + city+ "&delivery_time=" + pref_del_time1;  
			
			}else{
			add_address = getResources().getString(R.string.add_patient_address)+ "&customer_id="
						+ user_id + "&company="+ ""+"&customer_name=" + name1 +"&surname=" 
						+ surname1  +"&email=" + email + "&mobileNo=" + mobile + "&addr1=" + address1 + "&addr2=" 
						+ address2 + "&landmark=" + landmark1 + "&zip="+pincode+ "&country_id=" + country_id 
						+ "&zone_id=" + zone_id+ "&state_id=" + state + "&city_id=" + city+ "&delivery_time=" + pref_del_time1;  
			}
			WebJsonObjectRequest webjson=new WebJsonObjectRequest(Method.GET, add_address, new JSONObject(),new Listener<JSONObject>() 
					{
						...
						public void onResponse(JSONObject response) 
						{
							...
							if((messaqge.equals("inserted successfully"))){
								...
								add_id = Integer.parseInt(doctorsArr.getString("addr_id"));
					...
					if(defaultShip.isChecked()||defaultShip.isChecked()){	
					WebJsonObjectRequest adjson = new WebJsonObjectRequest(Method.GET, setDefaultAddUrl, new JSONObject(), new Response.Listener<JSONObject>() {
						...
						VolleyRequest.addJsonObjectRequest(ack, adjson);
								...
								if(content==2){
									adbean.setDefault_shipping(Integer.parseInt(adId));
									aList.add(adbean);
									ubean.setAddDatabean(aList);
									Utilities.writeCurrentUserToSharedPref(ack, ubean);
								}
		...
		webjson=new WebJsonObjectRequest(Method.GET, cityUrl, new JSONObject(),new Listener<JSONObject>() 
				{
					...
					public void onResponse(JSONObject response) 
					{
						...
						adapter2 = new ArrayAdapter<String>(ack	, android.R.layout.simple_dropdown_item_1line,cities)
	...
	public void getStates(){
		...
		webjson=new WebJsonObjectRequest(Method.GET,ste , new JSONObject(),new Listener<JSONObject>() 
				{
					@Override
					public void onResponse(JSONObject response) 
					{
