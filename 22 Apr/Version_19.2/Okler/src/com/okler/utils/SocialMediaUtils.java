	    	...
	    	switch(Okler.getInstance().getBookingType())
				 {
					...
					if(paList.size()<=0){
					Intent intent = new Intent(activity,EnterPatientInfoActivity.class);
					activity.startActivity(intent);
					}else{
						Intent intent = new Intent(activity,SelectPatientInfoActivity.class);
						activity.startActivity(intent);
					}
						break;
								...
								*/String message = responseObj.getString("message"); 
								...
								{							
									...
									if(sLogin == SocialLoginEnum.SIGNIN)
									{
										...
										{
											Utilities.writeUserStatusToSharedPref(activity, UserStatusEnum.LOGGED_IN_FB);
										}
										else if(logged_from == "LOGGED_IN_GOOGLE")
										{
											Utilities.writeUserStatusToSharedPref(activity, UserStatusEnum.LOGGED_IN_GOOGLE);
										}
										redirectUser();
									}
	 ...
	 public static void signOutOfGoogle()
	 {
	  ...
	  public class SignUpSocialLoginAsyncTask extends AsyncTask<String,Object,Object>{
		...
		private void WebCallForSocialLogin() {
			...
			String socal_call_back = activity.getResources().getString(R.string.serverUrl)+activity.getResources().getString(R.string.social_login_callback)+"salutation="+"&cust_id="+usrId+"&customer_name="+personName+"&email="+email;
			
			WebJsonObjectRequest webObjReq=new WebJsonObjectRequest(Method.GET, socal_call_back, new JSONObject(),new Response.Listener<JSONObject>() 
					{
