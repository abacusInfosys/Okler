...
public class VerifyPhoneNumDialog extends DialogFragment  {
		...
		private static int WAIT_TIMEOUT = 2000,TOTAL_WAIT_TIMEOUT=50000,CURRENT_TIMEOUT=2000;	
		Context diaContext;
		...
		{}
			public VerifyPhoneNumDialog(Activity context, String phoneNumber,boolean isFromNewSignin) {
		...
		/*	public class SignUpAsyncTask extends AsyncTask<String,Object,Object>{
			...
			public void webOtpCall() {
				final ProgressDialog ringProgressDialog = ProgressDialog.show(actCntx, "Verifying OTP ...", "", true);
    	        ringProgressDialog.setCancelable(false);
    	        
				otpUrl=actCntx.getString(R.string.get_otp_for_phoneNum1)+phoneNumberReceived+actCntx.getString(R.string.get_otp_for_phoneNum2);
				...
				},new Response.ErrorListener() 
				{
					...
					public void onErrorResponse(VolleyError error) {
						...
						//712016****
						Toast.makeText(actCntx,"No Network Connection",Toast.LENGTH_LONG).show();
						//dismiss();
				}
				...
				new Handler().postDelayed(new Runnable() {	   		            
		            ...
		            public void run() {
		            	...
		            	{
		            	//	Toast.makeText(actCntx, "Got otp.calling approved status", Toast.LENGTH_LONG).show();
		            		userBean= Utilities.getCurrentUserFromSharedPref(actCntx); //Okler.getInstance().getuDataBean();
							Gson gson=new Gson();
							resetPassNameValPair.add(new BasicNameValuePair("user_id", String.valueOf(userBean.getId())));
							resetPassNameValPair.add(new BasicNameValuePair("approved_status", String.valueOf(1)));						
							new ResetpassAsyncTask().execute("");	
		            	}
		            	
		            	else if(CURRENT_TIMEOUT > TOTAL_WAIT_TIMEOUT)
		            	{
		            	//	Toast.makeText(actCntx, "current timeout == wait timeout", Toast.LENGTH_LONG).show();
		            		Intent startAct = new Intent(actCntx,OtpConfirmationActivity.class);
		            		actCntx.startActivity(startAct);
		            		actCntx.finish();
		            	}
		            	else
		            	{
		            	//	Toast.makeText(actCntx, "wait for next 2sec", Toast.LENGTH_LONG).show();
		            		waitForNext2Sec();
		            	}
		            }
		        }, WAIT_TIMEOUT);				
			...
			public class ResetpassAsyncTask extends AsyncTask<String,Object,Object>{
				...
				protected Object doInBackground(String... params) {
					...
					String Result = String.valueOf(Utilities.RegisterNewUser(url,resetPassNameValPair));
					return Result;
			...
			protected void onPostExecute(Object result) {
				...
				try {
					...
					{								
						userBean=Okler.getInstance().getuDataBean();
						Utilities.writeUserStatusEnumToSharedPref(actCntx, UserStatusEnum.LOGGED_IN);
						Utilities.writeCurrentUserToSharedPref(actCntx, userBean);
						Intent success=new Intent(diaContext,ServiceCategoryActivity.class);
						startActivity(success);
						Toast.makeText(actCntx,"OTP is verified successfully",Toast.LENGTH_LONG).show();
						
						UsersDataBean ubean = Utilities.getCurrentUserFromSharedPref(actCntx);
						...
						String user_registration = actCntx.getResources().getString(R.string.serverUrl)+actCntx.getResources().getString(R.string.user_registration_url)+"salutation="+salutation+"&cust_id="+cust_id+"&customer_name="+name+"&email="+email;
						
						WebJsonObjectRequest webObjReq=new WebJsonObjectRequest(Method.GET, user_registration, new JSONObject(),new Response.Listener<JSONObject>() 
								{
