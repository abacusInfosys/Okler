	  ...
	  public void onClick(View v) {
		  ...
		  flag=validateEmail();
			if(flag==true)
			{
				...
				Utilities.writeToLogFIle("Got response. Phone "+phone+" Email"+email);
					  ...
					  WebJsonObjectRequest webjson=new WebJsonObjectRequest(Method.GET, isUserExist, new JSONObject(),new Listener<JSONObject>() 
								{
									...
									public void onResponse(JSONObject response) 
									{
										...
										Utilities.writeToLogFIle("Got response from msg");
										if(message.equals("User Available"))
										{
											Utilities.writeToLogFIle("Got response, User available");
											JSONArray jrray = responseObj.optJSONArray("result");
											Utilities.writeToLogFIle("Get json arr");
											JSONObject resObject = jrray.getJSONObject(0);
											Utilities.writeToLogFIle("Got response respo obj");
											setUserData(resObject);
											Utilities.writeToLogFIle("after set user data");
											
										String url1,url2,forgotPassUrl;
										Utilities.writeToLogFIle("String");
					url1 = c.getString(R.string.serverUrl);
					Utilities.writeToLogFIle("Got response from msg. URL1:"+url1);
					url2 = c.getString(R.string.forget_password);
					Utilities.writeToLogFIle("Got response from msg URL2:"+url2);
					forgotPassUrl = url1+url2+email; 
					Utilities.writeToLogFIle("Got response from msg.Forgot PWD:"+forgotPassUrl);
			WebJsonObjectRequest forgotjson = new WebJsonObjectRequest(Method.GET, forgotPassUrl, new JSONObject(), new Response.Listener<JSONObject>() {
				...
				public void onResponse(JSONObject response) {
					...
					try 
					{
						...
						if(result.equals("true"))
						{
							...
							Utilities.writeToLogFIle("Got response from msg.Result true");
							UsersDataBean ubean = Utilities.getCurrentUserFromSharedPref(c);
							Utilities.writeToLogFIle("Got response from msg.Result true. Ubean");
							String salutation =  ubean.getSalutation();
							salutation = URLEncoder.encode(salutation, "UTF-8");
							Utilities.writeToLogFIle("Got response from msg.Result true. Salutation:"+salutation);
							String name = ubean.getFname();
							name = URLEncoder.encode(name,"UTF-8");
							Utilities.writeToLogFIle("Got response from msg.Result true. Name:"+name);
							String email = ubean.getEmail();
							Utilities.writeToLogFIle("Got response from msg.Result true. emial:"+email);
							Utilities.writeToLogFIle("Got response from msg.ID:"+ubean.getId());
							String forgotpassUrl = c.getString(R.string.serverUrl)+c.getString(R.string.forgotPasswordUrl)+"salutation="+salutation+"&customer_name="+name+"&email="+email;
							Utilities.writeToLogFIle("Got response from msg. Forgot pass url:"+forgotpassUrl);
							WebJsonObjectRequest webObjReq=new WebJsonObjectRequest(Method.GET, forgotpassUrl, new JSONObject(),new Response.Listener<JSONObject>() 
									{
										@Override
										public void onResponse(JSONObject response) 
										{
											Log.i("contact us", "*****  mail sent*****");
											Utilities.writeToLogFIle("Got response from msg. Forgot pass on response");
										}
								...
								},new Response.ErrorListener() 
								{
										...
										//	Log.e("error", new String(error.networkResponse.data));
											...
											Utilities.writeToLogFIle("Got response from msg. Error response"+error.getStackTrace());
									}
					...
					Utilities.writeToLogFIle("Got response. before Password confirmation dialog");
					
					PasswordConfirmationDialog newpass = new PasswordConfirmationDialog(c);
					newpass.show();
					Utilities.writeToLogFIle("Got response. after Password confirmation dialog");
				...
				public void onErrorResponse(VolleyError error) {
					...
					Utilities.writeToLogFIle("Got response. on error response");
				}
			},true);
			VolleyRequest.addJsonObjectRequest(c, forgotjson);
							...
							{
								Toast.makeText(c, "User Not Available", Toast.LENGTH_LONG).show();
							}
	  ...
	  private void setUserData(JSONObject jobj1)
		{
		  ...
		  UsersDataBean ubean;
			...
			Utilities.writeToLogFIle("In SetUserData() salutation:"+ubean.getSalutation());
			ubean.setFname(jobj1.optString("firstname"));
			Utilities.writeToLogFIle("In SetUserData() first name"+ubean.getFname());
			ubean.setLname(jobj1.optString("lastname"));
			Utilities.writeToLogFIle("In SetUserData() lastname:"+ubean.getLname());
			ubean.setEmail(jobj1.optString("email"));
			Utilities.writeToLogFIle("In SetUserData() Email:"+ubean.getEmail());
			ubean.setPhone(jobj1.optString("phone"));
			Utilities.writeToLogFIle("In SetUserData() Phone:"+ubean.getPhone());
			ubean.setUserName(jobj1.optString("username"));
			Utilities.writeToLogFIle("In SetUserData() user name:"+ubean.getUserName());
			ubean.setUser_image(jobj1.optString("user_image"));
			Utilities.writeToLogFIle("In SetUserData() user image:"+ubean.getUser_image());
			ubean.setGender(jobj1.optString("gender"));
			Utilities.writeToLogFIle("In SetUserData() gender:"+ubean.getGender());
			...
			Utilities.writeToLogFIle("In SetUserData() ref code:"+ubean.getRefCode());
			ubean.setCompany(jobj1.optString("company"));
			Utilities.writeToLogFIle("In SetUserData() company:"+ubean.getCompany());
			ubean.setAvailable_status(Integer.parseInt(jobj1.optString("available_status")));
			Utilities.writeToLogFIle("In SetUserData() avail status:"+ubean.getAvailable_status());
			ubean.setCurr_balance(Float.parseFloat(jobj1.optString("curr_balance")));
			Utilities.writeToLogFIle("In SetUserData() curr bal:"+ubean.getCurr_balance());
			...
			Utilities.writeToLogFIle("In SetUserData() blood group:"+ubean.getBlood_group());
			...
			if(!jobj1.isNull("userAddr"))
			{
				...
				for (int i = 0; i < length; i++) {
					...
					Utilities.writeToLogFIle("In SetUserData() Addresses fname:"+adbean.getFirstname());
					adbean.setLastname(jobj2.optString("surname"));
					Utilities.writeToLogFIle("In SetUserData() Addresses fname:"+adbean.getLastname());
					adbean.setAddress1(jobj2.optString("addr1"));
					Utilities.writeToLogFIle("In SetUserData() Addresses set addr1:"+adbean.getAddress1());
					adbean.setAddress2(jobj2.optString("addr2"));
					Utilities.writeToLogFIle("In SetUserData() Addresses addr2:"+adbean.getAddress2());
					adbean.setZip(jobj2.optString("zip"));
					Utilities.writeToLogFIle("In SetUserData() Addresses zip:"+adbean.getZip());
					adbean.setCountry_id(jobj2.optString("country_id"));
					Utilities.writeToLogFIle("In SetUserData() Addresses country id:"+adbean.getCountry_id());
					adbean.setAddr_id(jobj2.optString("addr_id"));
					Utilities.writeToLogFIle("In SetUserData() Addresses addr id:"+adbean.getAddr_id());
					adbean.setCity(jobj2.optString("city_name"));
					Utilities.writeToLogFIle("In SetUserData() Addresses city name:"+adbean.getCity());
					adbean.setState(jobj2.optString("state_name"));
					Utilities.writeToLogFIle("In SetUserData() Addresses state name:"+adbean.getState());
					adbean.setCity_id(jobj2.optString("city_id"));
					Utilities.writeToLogFIle("In SetUserData() Addresses city id:"+adbean.getCity_id());
					adbean.setState_id(jobj2.optString("state_id"));
					Utilities.writeToLogFIle("In SetUserData() Addresses state id:"+adbean.getState_id());
					adbean.setCountry(jobj2.optString("country_name"));
					Utilities.writeToLogFIle("In SetUserData() Addresses country:"+adbean.getCountry());
					adbean.setDefault_billing(jobj2.optInt("default_billing"));
					Utilities.writeToLogFIle("In SetUserData() Addresses default:"+adbean.getDefault_billing());
					adbean.setDefault_shipping(jobj2.optInt("default_shiping"));
					Utilities.writeToLogFIle("In SetUserData() Addresses shipping:"+adbean.getDefault_shipping());
					addList.add(adbean);
				}
