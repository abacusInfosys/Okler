		...
		proceed.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View v) {
				...
				for(int i =0;i<nameValuePairs.size();i++){
					Utilities.writeToLogFIle("Prdel: nvpairs "+nameValuePairs.get(i).getName()+"  "+nameValuePairs.get(i).getValue());
				}
	...
	public class UploadPrescAsyncTask extends AsyncTask<String, Object, Object>{
		...
		protected Object doInBackground(String... params) {
			...
			//	httpPost.setHeader("Accept","application/Json");
				...
				e.printStackTrace();
				Utilities.writeToLogFIle("Prdel: UpResult UnsupportedEncodingException "+e.getStackTrace());
				return null;
			} catch (ClientProtocolException e) {				
				e.printStackTrace();
				Utilities.writeToLogFIle("Prdel: UpResult ClientProtocolException "+e.getStackTrace());
				return null;
				
			} catch (IOException e) {				
				e.printStackTrace();
				Utilities.writeToLogFIle("Prdel: UpResult IOException "+e.getStackTrace());
				return null;
			} 
			return null;
		}
			...
			if(resu.contains("result\":\"true"))
			{
				...
				WebJsonObjectRequest webObjReq=new WebJsonObjectRequest(Method.GET, upload_pres_email, new JSONObject(),new Response.Listener<JSONObject>() 
						...
						{	
							...
							// salutation=&cust_id=23&customer_name=naveen&email=naveen.kumar.ayyapusetty@okler.com&presc_id=1234
							
							String call_me_url = getResources().getString(R.string.serverUrl)+getResources().getString(R.string.call_me_to_discuss_url)+"salutation="+salutation+"&cust_id="+cust_id+"&customer_name="+customer_name+"&email="+email+"&presc_id="+presc_id;

							WebJsonObjectRequest callMeReq=new WebJsonObjectRequest(Method.GET, call_me_url, new JSONObject(),new Response.Listener<JSONObject>() 
									{
