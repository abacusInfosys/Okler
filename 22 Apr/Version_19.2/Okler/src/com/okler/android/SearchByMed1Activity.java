...
public class SearchByMed1Activity extends BaseActivity implements
	...
	private void getMedsForDisease() {
		...
		getMedsJson = new WebJsonObjectRequest(Method.GET,
				getAllMedsByDisease, new JSONObject(),
				new Response.Listener<JSONObject>() {
	...
	private void getAllMeds() {
		String newURL = setMedsURL(pageNo, searchText, "", prodtype, "",
				brand_id, "", "");
		getMedsJson = new WebJsonObjectRequest(Method.GET,
				newURL, new JSONObject(), new Response.Listener<JSONObject>() {
	...
	public boolean onQueryTextChange(String searchTxt) {
		...
		if(getMedsJson != null)
			getMedsJson.cancel();
		 getMedsJson = new WebJsonObjectRequest(Method.GET,
				newURL, new JSONObject(), new Listener<JSONObject>() {
					@Override
					public void onResponse(JSONObject response) {
						// TODO Auto-generated method stub
						showProgress(false);
